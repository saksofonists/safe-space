    ...
    private void Create2DMap()
    {
        ...
        for (int i = 0; i < height; i++)
        {
            for (int j = 0; j < width; j++)
            {
                ...
                var colliders = Physics2D.OverlapPointAll(new Vector2(x, y));

//                RaycastHit2D[] hit = Physics2D.LinecastAll(
//                    new Vector2(x, y),
...
//                    );
                ...
                {
                    if (DisallowedTags.Contains(c.transform.tag))
                    {
//                        if (h.point.z < maxZ)
//                        {
...
//                        }
                    ...
                    else if (IgnoreTags.Contains(c.transform.tag))
                    {
